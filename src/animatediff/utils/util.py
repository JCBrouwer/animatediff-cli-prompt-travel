...
def save_video(video: Tensor, save_path: PathLike, fps: int = 8, format: str = "mp4"):
    ...
    if format == "mp4":
        ...
        video_writer = cv2.VideoWriter(str(save_path), fourcc, fps, frames[0].shape[:2][::-1])
        for frame in tqdm(frames, desc=f"Saving video to {save_path.stem}"):
