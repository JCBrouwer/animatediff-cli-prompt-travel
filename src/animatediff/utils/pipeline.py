...
def send_to_device(
    ...
    if compile:
        if not isinstance(pipeline.unet, dynamo.OptimizedModule):
            ...
            pipeline.unet = torch.compile(pipeline.unet, backend="inductor", mode="reduce-overhead")
        else:
...
def get_context_params(
    length: int, context: Optional[int] = None, overlap: Optional[int] = None, stride: Optional[int] = None
):
    if context is None:
